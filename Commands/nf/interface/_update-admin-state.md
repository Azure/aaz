# [Command] _nf interface update-admin-state_
...
- Update admin state of Network Interface
    ```bash
        nf interface update-admin-state -g "example-rg" --network-device-name "example-device" --resource-name "example-interface" --state "Enable"
    ```
