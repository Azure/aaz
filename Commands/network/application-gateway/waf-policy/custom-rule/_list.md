# [Command] _network application-gateway waf-policy custom-rule list_
...
- List application gateway WAF policy custom rules.
    ```bash
        network application-gateway waf-policy custom-rule list --policy-name MyPolicy --resource-group MyResourceGroup
    ```
