# [Command] _network lb address-pool tunnel-interface remove_
...
- Remove one tunnel interface from the load balance tunnel interface pool.
    ```bash
        network lb address-pool tunnel-interface remove -g MyResourceGroup --lb-name MyLb --address-pool MyAddressPool --index 0
    ```
