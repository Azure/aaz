...
- Set the record type on a traffic manager profile where it is not already set.
    ```bash
        network traffic-manager profile update -g MyResourceGroup -n MyTmProfile --record-type A
    ```
