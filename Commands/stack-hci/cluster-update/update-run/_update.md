# [Command] _stack-hci cluster-update update-run update_
...
- Update update run
    ```bash
        stack-hci cluster-update update-run update --cluster-name name -g rg  -n updates-run-name --update-name name --progress "{name:cli_update_test,description:update_test}"
    ```
