# [Command] _cloud-health healthmodel identity assign_
...
- Assign system-assigned identity to a health model
    ```bash
        healthmodel identity assign --resource-group myRG --name myModel --system-assigned
    ```

- Assign a user-assigned identity to a health model
    ```bash
        healthmodel identity assign --resource-group myRG --name myModel --user-assigned /subscriptions/.../userAssignedIdentities/myUai
    ```
