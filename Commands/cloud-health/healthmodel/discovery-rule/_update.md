# [Command] _cloud-health healthmodel discovery-rule update_
...
- Disable recommended signals on a discovery rule
    ```bash
        healthmodel discovery-rule update --resource-group myRG --health-model-name myModel --name vmDiscovery --add-recommended-signals Disabled
    ```
