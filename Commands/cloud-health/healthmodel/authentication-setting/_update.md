# [Command] _cloud-health healthmodel authentication-setting update_
...
- Update the display name of an authentication setting
    ```bash
        healthmodel authentication-setting update --resource-group myRG --health-model-name myModel --name myAuth --display-name "Auth for production"
    ```
