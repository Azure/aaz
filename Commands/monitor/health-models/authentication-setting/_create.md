...
- Create an authentication setting backed by the model's system-assigned identity
    ```bash
        monitor health-models authentication-setting create --resource-group myRG --health-model-name myModel --name myAuth --managed-identity SystemAssigned
    ```

- Create an authentication setting backed by a user-assigned identity
    ```bash
        monitor health-models authentication-setting create --resource-group myRG --health-model-name myModel --name myAuth --managed-identity /subscriptions/.../userAssignedIdentities/myUai
    ```
