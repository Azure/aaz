...
- Push an external health report for a signal on an entity
    ```bash
        monitor health-models entity ingest-health-report --resource-group myRG --health-model-name myModel --entity-name webTier --signal-name synthetic-probe --health-state Healthy --value 1 --expires-in-minutes 10 --additional-context "HTTP 200 from synthetic probe"
    ```
