...
- Create a health model
    ```bash
        monitor health-models create --resource-group myRG --name myModel --location eastus
    ```

- Create a health model with a system-assigned managed identity
    ```bash
        monitor health-models create --resource-group myRG --name myModel --location eastus --mi-system-assigned
    ```
