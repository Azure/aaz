...
- Upsert DNS security rule with managed domain list
    ```bash
        dns-resolver policy dns-security-rule update --resource-group sampleResourceGroup --policy-name sampleDnsResolverPolicy --dns-security-rule-name sampleDnsSecurityRule --action "{action-type:Block}" --rule-state Enabled --priority 100 --managed-domain-lists "[AzureDnsThreatIntel]" --tags "{key1:value1}"
    ```
