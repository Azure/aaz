# [Command] _workload-manager workload-space runtime-link update_
...
- Update runtime link capacity
    ```bash
        workload-manager workload-space runtime-link update --resource-group rg-workload --space-name managed-agents-prod --link-name default --capacity-profile "{minimum-nodes:2,maximum-nodes:150}"
    ```
